...
// Uses a map and a bitmap (size M) to keep sample when growing
//

#ifndef CDSLIB_GMHLZ78S2_H
#define CDSLIB_GMHLZ78S2_H
...
namespace cdslib {
    ...
    private:
        ...
        size_type bits_d;
				size_type bits_H;
        size_type MAX_M;
        double factor, gf;                  //factor: overload factor. gf: grow factor in each iteration
    ...
    public:
        ...
        compress_file(std::string file_name, std::string out_file, size_type s,
                      double _factor = 0.05, size_type _bits_d = 0, double _gf = 2.0) {
            ...
            n = (size_type) std::sqrt(len_file); //best case-->initial case
       ...
       //! Decompress the stored text into out_file. Note that first the data structure must be loaded
        ...
        load(std::istream &in) {
            ...
            parameters = LoadValue<size_type>(in, 9);
    ...
    private:
        ...
        ghlz78S2(size_type _MAX_M, size_type _M, size_type _sigma,
               size_type _alpha, size_type _bits_d, double _factor, size_type _bits_L, double _gf = 2) {
            M = _M;
            MAX_M = _MAX_M;
            sigma = _sigma;
            alpha = _alpha;
            ...
            beta = mod_mul_inv(alpha, P);
            max_h = (P + M) / M;
						bits_H =  sdsl::bits::hi(max_h) + 1;
						H = sdsl::int_vector<>(M, max_h, bits_H);
        ...
        grow_tables(std::ostream &out) {
            ...
            ghlz78S2<t_width, D_type> new_ds(MAX_M, M_prime, sigma, alpha, bits_d, factor, bits_L, gf);
            ...
            uint64_t *reading_buffer;
            uint64_t pos_reading_buffer = 0;
						size_type pos_to_update_sample;
            max_size_bytes = bits_L * (cdslib::BUFFER_SIZE / (8 * bits_L));
            out.seekp(0, out.beg); //move to the start
            SaveValue(out, (int64_t) 0); //save space to store the start of the data structure
            
						//alloc space to store the samples
						sdsl::int_vector<> W = sdsl::int_vector<>(M, 0, 1);
						std::cout << "M : " << M << std::endl; 
						W[0] = 1; //sample the root
						std::map<size_type, size_type> W_values;
                ...
                h_pos = cdslib::get_field(reading_buffer, pos_reading_buffer, pos_reading_buffer + bits_L - 1);
                auto phrase = extract_not_sampled(h_pos, W, W_values, new_ds.M, current_node_pos, pos_to_update_sample); //extract part of the phrase that is not sampled 
                len_p = phrase.size();
								if (pos_to_update_sample != 0 and len_p > 0) {  //insert the first bits_H nodes in the new table
									for (size_type i=0; i < bits_H; i++) {
										node_value =  current_node_pos * sigma + (phrase[len_p - 1]);
										current_node_pos = new_ds.insert(node_value, out);
										--len_p;
									}
									W[pos_to_update_sample] = 1;
									W_values[pos_to_update_sample] = current_node_pos;
								}
								while (len_p > 0) { //now insert the rest in the new tables
									node_value = current_node_pos * sigma + (phrase[len_p - 1]);
									current_node_pos = new_ds.insert(node_value, out);
									-- len_p;
								}
								pos_reading_buffer += bits_L; //move to next phrase in the buffer
								if (pos_reading_buffer == read_size_bits) {
									delete[] reading_buffer;
									pos_reading_buffer = 0;
									need_data = true;
            ...
            //swap data structures
						size_type ram_old_ds = size_in_bytes();
						std::cout << "old ds " << ram_old_ds << "  bytes" << std::endl;						
						size_type ram_new_ds = new_ds.size_in_bytes();
            ...
            this->swap(new_ds);
				...
				//!Extracts the part of the string path until finding a sampled node
				std::vector<value_type>
				extract_not_sampled(size_type h_pos, sdsl::int_vector<> &W,  std::map<size_type, size_type> &W_values, size_type max_sampled, size_type &starting_node, size_type &pos_to_update) {
					std::vector<value_type> phrase;
					size_type window_size = bits_H;
					std::vector<size_type> window_pos(window_size, 0);
					size_type current_h_value, actual_h_pos, old_h_pos;
					size_type mod_P_value, node_value, d_value;
					size_type distance_to_sample = 0;
					pos_to_update = 0;
					while (W[h_pos] == 0) { //while we are not in a sampled position
						distance_to_sample ++;
						window_pos[distance_to_sample % window_size] = h_pos;
						old_h_pos = h_pos; //this is new
						current_h_value = H[h_pos];
						actual_h_pos = h_pos;
						d_value = D.getValue(h_pos);
					...
					if(distance_to_sample >= 2 * bits_H) {//distance >= aprox 2*log_2(sigma) -> update sample at bits_H from last sample
						pos_to_update = window_pos[(distance_to_sample + 1) % window_size];  //Position in the sample that need to be stored          
					}
					if(h_pos == 0)
						starting_node = 0;
					else {
						auto it = W_values.find(h_pos); // assume that always find it
						starting_node = it->second; // W_values[h_pos] get position of the last sampled node in M'
					}
					return phrase;
				}
