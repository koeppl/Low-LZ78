...
#ifndef CDSLIB_GHLZ78_S_H
#define CDSLIB_GHLZ78_S_H
...
namespace cdslib {
    ...
    private:
        ...
        ghlz78_sample(size_type _MAX_M, size_type _M, size_type _sigma,
        ...
        grow_tables(std::ostream &out) {
            ...
            size_type M_prime = (size_type)std::min(MAX_M, 2 * M);  //in practice is better to move by 2 than by (1+factor)
						ghlz78_sample<t_width, D_type> new_ds(MAX_M, M_prime, sigma, alpha, bits_d, factor, bits_L);
            ...
            SaveValue(out, (int64_t) 0); //save space to store the start of the data structure
						...
						sdsl::int_vector<> sampled = sdsl::int_vector<>(M, new_ds.M, sdsl::bits::hi(new_ds.M) + 1);
						sampled[0] = 0; //sample the root
						while (cont <  number_of_phrases) {
                if (need_data) {
                    read_space = end_pos - current_pos;
                    if (read_space > max_size_bytes)
                ...
                h_pos = cdslib::get_field(reading_buffer, pos_reading_buffer, pos_reading_buffer + bits_L - 1);
								...
								auto phrase = extract_not_sampled(h_pos, sampled, new_ds.M, current_node_pos, pos_to_update_sample); //extract part of the phrase that is not sampled    
								len_p = phrase.size();
								if (pos_to_update_sample != 0 and len_p > 0) {  //insert the first bits_H nodes in the new table
									...
									sampled[pos_to_update_sample] = current_node_pos;
								}
            ...
            size_type ram_old_ds = size_in_bytes();
						...
						this->swap(new_ds);
						//sampled information is free when method reach the end
						std::cout << "Sampled used: " << sdsl::size_in_bytes(sampled) <<  " bytes" << std::endl;
						std::cout << "old_ds: " << ram_old_ds <<  " bytes" << std::endl;
						std::cout << "new_ds: " << ram_new_ds <<  " bytes" << std::endl << std::endl;
        }

				//!Extracts the part of the string path until finding a sampled node
				std::vector<value_type>
				extract_not_sampled(size_type h_pos, sdsl::int_vector<> &sampled, size_type max_sampled, size_type &starting_node, size_type &pos_to_update) {
					std::vector<value_type> phrase;
					...
					pos_to_update = 0;
					while (sampled[h_pos] == max_sampled) { //while we are not in a sampled position
						distance_to_sample ++;
